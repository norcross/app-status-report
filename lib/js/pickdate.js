...
(function($) {
  ...
  function will_pickdate(element, index, options) {
    ...
    this.clone = this.element
            ...
            .data('will_pickdate', true)
            .removeAttr('name')
            .attr('id', this.element.attr('id') + '-display')
  ...
  will_pickdate.prototype = {
    ...
    constructPicker: function() {
      ...
      title_cont.append($('<span class="title-text"/>'));
    ...
    renderTime: function() {
      ...
      container.append($('<input type="submit" value="Select" class="button button-primary button-large ok"/>').click($.proxy(function(event) {
        event.stopPropagation();
    ...
    renderTitle: function(text){
        ...
        this.picker.find('.title-text').text(text);
    },
